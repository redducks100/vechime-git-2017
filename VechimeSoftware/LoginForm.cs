    ...
    public partial class LoginForm : Form
    {
        ...
        private void CheckForUpdates()
        {
            if(latestVersion != null)
            {
                ...
                Version version = new Version(latestVersion.Version);
                if (version > currentVersion)
                {
                    if(MessageBox.Show("A aparut o versiune noua: " + latestVersion.Version+".\nDoriti sa actualizati?","Update",MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        using (DownloadUpdateForm duf = new DownloadUpdateForm(this, latestVersion))
                        {
                            if (duf.ShowDialog() != DialogResult.OK)
                            {
                                MessageBox.Show(" Programul nu a fost actualizat!\nVa rugam verificati conexiunea la internet si incercati din nou.", "Eroare", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                            else
                            {
                                ...
                                process.StartInfo.Arguments = "/i " + updateFilePath; //+" /quiet /qn /norestart";
                                process.Start();
                                Environment.Exit(0);
                            }
                        }
        ...
        private VersionData GetLatestVersion()
        {
            Version currentVersion = Assembly.GetExecutingAssembly().GetName().Version;

            var db = mongoDB.GetDatabase(databaseName);
            var col = db.GetCollection<VersionData>("VersionsData");
            var result = col.Find(x=> true).ToListAsync().Result;

            VersionData lastVersion = new VersionData();
            lastVersion.Version = currentVersion.ToString();
            for(int i=0;i<result.Count;i++)
            {
                if (new Version(result[i].Version) >= new Version(lastVersion.Version))
        ...
        private void Connect(string username, string pass)
        {
            ...
            {
                using (MainForm main = new MainForm(this, false, startUp))
                {
            ...
            {
                using (MainForm main = new MainForm(this, true, startUp))
                {
